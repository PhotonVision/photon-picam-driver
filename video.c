...
#define CAM_WHITE_BALANCE_CONTROL                                              \
  OMX_WhiteBalControlAuto                    // OMX_WHITEBALCONTROLTYPE
...
static void omx_die(OMX_ERRORTYPE error, const char *message, ...) {
  ...
  case OMX_ErrorHardware:
    e = "hardware error";
    break;
  /* That's all I've encountered during hacking so let's not bother with the
   * rest... */
  default:
    e = "(no description)";
  }
...
static void dump_event(OMX_HANDLETYPE hComponent, OMX_EVENTTYPE eEvent,
  ...
  say("Received event 0x%08x %s, hComponent:0x%08x, nData1:0x%08x, "
      "nData2:0x%08x",
...
static const char *dump_color_format(OMX_COLOR_FORMATTYPE c) {
  ...
  case OMX_COLOR_FormatYUV411PackedPlanar:
    return "OMX_COLOR_FormatYUV411PackedPlanar: Planes fragmented when a frame "
           "is split in multiple buffers";
  case OMX_COLOR_FormatYUV420Planar:
    return "OMX_COLOR_FormatYUV420Planar: Planar YUV, 4:2:0 (I420)";
  case OMX_COLOR_FormatYUV420PackedPlanar:
    return "OMX_COLOR_FormatYUV420PackedPlanar: Planar YUV, 4:2:0 (I420), "
           "planes fragmented when a frame is split in multiple buffers";
  case OMX_COLOR_FormatYUV420SemiPlanar:
    return "OMX_COLOR_FormatYUV420SemiPlanar, Planar YUV, 4:2:0 (NV12), U and "
           "V planes interleaved with first U value";
  case OMX_COLOR_FormatYUV422Planar:
    return "OMX_COLOR_FormatYUV422Planar";
  case OMX_COLOR_FormatYUV422PackedPlanar:
    return "OMX_COLOR_FormatYUV422PackedPlanar: Planes fragmented when a frame "
           "is split in multiple buffers";
  ...
  case OMX_COLOR_FormatYUV420PackedSemiPlanar:
    return "OMX_COLOR_FormatYUV420PackedSemiPlanar: Planar YUV, 4:2:0 (NV12), "
           "planes fragmented when a frame is split in multiple buffers, U and "
           "V planes interleaved with first U value";
  case OMX_COLOR_FormatYUV422PackedSemiPlanar:
    return "OMX_COLOR_FormatYUV422PackedSemiPlanar: Planes fragmented when a "
           "frame is split in multiple buffers";
...
static void dump_portdef(OMX_PARAM_PORTDEFINITIONTYPE *portdef) {
  say("Port %d is %s, %s, buffers wants:%d needs:%d, size:%d, pop:%d, "
      "aligned:%d",
  ...
  case OMX_PortDomainImage:
    say("Image type:\n"
        ...
        "\tCodec:\t\t%s\n"
        "\tColor:\t\t%s\n",
        imgdef->nFrameWidth, imgdef->nFrameHeight, imgdef->nStride,
        imgdef->nSliceHeight,
...
static void init_component_handle(const char *name, OMX_HANDLETYPE *hComponent,
                                  OMX_PTR pAppData,
...
static void signal_handler(int signal) { want_quit = 1; }
...
void *video_decode_test(void *arg) {
  ...
  OMX_U32 screen_width = state->screenWidth,
          screen_height = state->screenHeight;
  if (graphics_get_display_size(DISPLAY_DEVICE, &screen_width, &screen_height) <
      0) {
  ...
  cstate.nWaitMask = 1;
  if ((r = OMX_SetConfig(ctx.clock, OMX_IndexConfigTimeClockState, &cstate)) !=
      OMX_ErrorNone) {
    omx_die(r, "Failed to request camera device number parameter change "
               "callback for camera");
  }
  ...
  if ((r = OMX_SetConfig(ctx.camera, OMX_IndexConfigRequestCallback,
                         &cbtype)) != OMX_ErrorNone) {
    omx_die(r, "Failed to request camera device number parameter change "
               "callback for camera");
  }
  ...
  if ((r = OMX_SetConfig(ctx.camera, OMX_IndexConfigVideoFramerate,
    ...
    omx_die(r, "Failed to set framerate configuration for camera preview "
               "output port 70");
  }
  framerate.nPortIndex = 71;
  if ((r = OMX_SetConfig(ctx.camera, OMX_IndexConfigVideoFramerate,
                         &framerate)) != OMX_ErrorNone) {
    omx_die(r, "Failed to set framerate configuration for camera video output "
               "port 71");
  }
  // Configure sharpness
  ...
  if ((r = OMX_SetConfig(ctx.camera, OMX_IndexConfigCommonSharpness,
                         &sharpness)) != OMX_ErrorNone) {
  ...
  if ((r = OMX_SetConfig(ctx.camera, OMX_IndexConfigCommonContrast,
                         &contrast)) != OMX_ErrorNone) {
  ...
  say("Setting up tunnel from camera preview output port 70 to null sink input "
      "port 240...");
  if ((r = OMX_SetupTunnel(ctx.clock, 80, ctx.camera, 73)) != OMX_ErrorNone) {
    omx_die(r, "Failed to setup tunnel between camera preview output port 70 "
               "and null sink input port 240");
  }
  // Tunnel camera preview output port and null sink input port
  say("Setting up tunnel from camera preview output port 70 to null sink input "
      "port 240...");
  ...
  say("Setting up tunnel from camera video output port 71 to render input port "
      "220...");
  if ((r = OMX_SetupTunnel(ctx.camera, 71, ctx.render, 220)) != OMX_ErrorNone) {
    omx_die(r, "Failed to setup tunnel between camera video output port 71 and "
               "render input port 90");
  }

  // Switch components to idle state
  say("Switching state of the camera component to idle...");
  if ((r = OMX_SendCommand(ctx.clock, OMX_CommandStateSet, OMX_StateIdle,
                           NULL)) != OMX_ErrorNone) {
    omx_die(r, "Failed to switch state of the camera component to idle");
  }
  block_until_state_changed(ctx.clock, OMX_StateIdle);
  say("Switching state of the camera component to idle...");
  if ((r = OMX_SendCommand(ctx.camera, OMX_CommandStateSet, OMX_StateIdle,
                           NULL)) != OMX_ErrorNone) {
    omx_die(r, "Failed to switch state of the camera component to idle");
  }
  block_until_state_changed(ctx.camera, OMX_StateIdle);
  say("Switching state of the render component to idle...");
  if ((r = OMX_SendCommand(ctx.render, OMX_CommandStateSet, OMX_StateIdle,
                           NULL)) != OMX_ErrorNone) {
  ...
  // Enable ports
  say("Enabling ports...");
  if ((r = OMX_SendCommand(ctx.clock, OMX_CommandPortEnable, 80, NULL)) !=
      OMX_ErrorNone) {
    omx_die(r, "Failed to enable camera input port 73");
  }
  block_until_port_changed(ctx.clock, 80, OMX_TRUE);
  ...
  if ((r = OMX_SendCommand(ctx.render, OMX_CommandPortEnable, 221, NULL)) !=
      OMX_ErrorNone) {
  ...
  if ((r = OMX_UseEGLImage(ctx.render, &ctx.eglBuffer, 221, NULL,
                           ctx.eglImage)) != OMX_ErrorNone) {
  ...
  if ((r = OMX_SendCommand(ctx.clock, OMX_CommandStateSet, OMX_StateExecuting,
                           NULL)) != OMX_ErrorNone) {
  ...
  if ((r = OMX_SendCommand(ctx.render, OMX_CommandStateSet, OMX_StateExecuting,
                           NULL)) != OMX_ErrorNone) {
  ...
  if ((r = OMX_SendCommand(ctx.render, OMX_CommandStateSet, OMX_StateLoaded,
                           NULL)) != OMX_ErrorNone) {
