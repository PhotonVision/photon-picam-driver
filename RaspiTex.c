...
static int raspitex_draw(RASPITEX_STATE *state, MMAL_BUFFER_HEADER_T *buf) {
  ...
  if (buf) {
    ...
    if (state->ops.update_texture) {
      ...
      if (rc != 0) {
        vcos_log_error("%s: Failed to update RGBX texture %d", VCOS_FUNCTION,
                       rc);
        goto end;
      }
    }
    ...
    if (state->ops.update_y_texture) {
      ...
      if (rc != 0) {
        vcos_log_error("%s: Failed to update Y' plane texture %d",
                       VCOS_FUNCTION, rc);
        goto end;
      }
    }
    ...
    if (state->ops.update_u_texture) {
      ...
      if (rc != 0) {
        vcos_log_error("%s: Failed to update U plane texture %d", VCOS_FUNCTION,
                       rc);
        goto end;
      }
    }
 ...
 * Ensures camera preview is supplied with buffers and sends preview frames to
 * GL.
...
int raspitex_configure_preview_port(RASPITEX_STATE *state,
  ...
  state->preview_pool = mmal_port_pool_create(
      preview_port, preview_port->buffer_num, preview_port->buffer_size);
...
int raspitex_start(RASPITEX_STATE *state) {
  ...
  vcos_log_trace("%s", VCOS_FUNCTION);
  status = vcos_thread_create(&state->preview_thread, "preview-worker", NULL,
                              preview_worker, state);

  if (status != VCOS_SUCCESS)
    vcos_log_error("%s: Failed to start worker thread %d", VCOS_FUNCTION,
                   status);
