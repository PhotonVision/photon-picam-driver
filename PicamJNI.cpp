...
JNIEXPORT jlong JNICALL Java_org_photonvision_raspi_PicamJNI_initVCSMInfo
  ...
  if (state.vcsmBufWidth > 2048 || state.vcsmBufHeight > 2048) {
    std::cerr << "VCSM doesn't support buffers with a width or height greater than 2048\n";
    return 0;
  }
...
JNIEXPORT jboolean JNICALL Java_org_photonvision_raspi_PicamJNI_createCamera
  (JNIEnv *, jclass, jint width, jint height, jint fps) {
    try {
      ...
      // Init Broadcom libs (this has probably already been done by JOGL, but we'll be defensive)
      bcm_host_init();
...
JNIEXPORT void JNICALL Java_org_photonvision_raspi_PicamJNI_grabFrame
  ...
  if (s.width != state.screenWidth || s.height != state.screenHeight || outImage.channels() != 1) {
    std::cerr << "Passed Mat is incorrectly sized or has more than one channel\n";
    return;
  }

  // Make the buffer CPU addressable with host cache enabled
  VCSM_CACHE_TYPE_T cacheType;
  unsigned char* vcsmBuffer = static_cast<unsigned char*>(vcsm_lock_cache(state.vcsmInfo.vcsm_handle, VCSM_CACHE_TYPE_HOST, &cacheType));
  if (!vcsmBuffer) {
    std::cerr << "Failed to lock VCSM buffer\n";
    return;
  }

  // First we copy out every fourth byte (we need a single channel image, but the shared memory we map can only RGBA--i.e. 4 bytes per pixel)
  int bound = state.vcsmBufWidth * (state.screenHeight - 1) + state.screenWidth; // IMPORTANT! Bound must be calculated here or GCC doesn't vectorize the loop
  for (int i = 0; i < bound; i++) {
    // We keep an intermediate buffer because memcpy can't deal with overlapping copies (memmove does, but it slower)
  ...
  // Crop out the bits we don't need (this is needed because the shared memory must be a power of two in both width and height)
  unsigned char* outBuf = outImage.ptr();
  for (int y = 0; y < state.screenHeight; y++) {
    std::memcpy(outBuf + y * state.screenWidth, state.intermediateBuffer + y * state.vcsmBufWidth, state.screenWidth);
  }
}
