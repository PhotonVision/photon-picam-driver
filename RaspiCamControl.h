...
typedef struct raspicam_camera_parameters_s {
  ...
  char annotate_string
      [MMAL_CAMERA_ANNOTATE_MAX_TEXT_LEN_V2]; /// String to use for annotate -
                                              /// overrides certain bitmask
                                              /// settings
...
typedef enum { ZOOM_IN, ZOOM_OUT, ZOOM_RESET } ZOOM_COMMAND_T;
...
int raspicamcontrol_set_annotate(MMAL_COMPONENT_T *camera, const int bitmask,
...
raspicamcontrol_get_metering_mode(MMAL_COMPONENT_T *camera);
...
MMAL_PARAM_THUMBNAIL_CONFIG_T
raspicamcontrol_get_thumbnail_parameters(MMAL_COMPONENT_T *camera);
MMAL_PARAM_EXPOSUREMODE_T
raspicamcontrol_get_exposure_mode(MMAL_COMPONENT_T *camera);
MMAL_PARAM_FLICKERAVOID_T
raspicamcontrol_get_flicker_avoid_mode(MMAL_COMPONENT_T *camera);
