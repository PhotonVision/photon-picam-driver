...
int raspicamcontrol_set_all_parameters(
  ...
  result +=
      raspicamcontrol_set_flicker_avoid_mode(camera, params->flickerAvoidMode);
  result +=
  ...
  result += raspicamcontrol_set_annotate(
      ...
      params->annotate_bg_colour, params->annotate_justify, params->annotate_x,
      params->annotate_y);
  result += raspicamcontrol_set_gains(camera, params->analog_gain,
                                      params->digital_gain);
...
int raspicamcontrol_zoom_in_zoom_out(MMAL_COMPONENT_T *camera,
  ...
  if (mmal_port_parameter_get(camera->control, &crop.hdr) != MMAL_SUCCESS) {
    vcos_log_error(
        "mmal_port_parameter_get(camera->control, &crop.hdr) failed, skip it");
    return 0;
  }
...
int raspicamcontrol_set_focus_window(MMAL_COMPONENT_T *camera,
                                     int focus_window) {
  if (!camera)
    return 1;

  return mmal_status_to_int(mmal_port_parameter_set_boolean(
      camera->control, MMAL_PARAMETER_DRAW_BOX_FACES_AND_FOCUS, focus_window));
}
  ...
  if (settings) {
    ...
    /*if (settings & (ANNOTATE_APP_TEXT | ANNOTATE_USER_TEXT)) {
      if ((settings & (ANNOTATE_TIME_TEXT | ANNOTATE_DATE_TEXT)) &&
        ...
        strftime(annotate.text, MMAL_CAMERA_ANNOTATE_MAX_TEXT_LEN_V3, string,
                 &tm);
        process_datetime = 0;
      } else {
...
void raspicamcontrol_check_configuration(int min_gpu_mem) {
  ...
  if (!supported)
    ...
    vcos_log_error(
        "Only %dM of gpu_mem is configured. Try running \"sudo raspi-config\" "
        "and ensure that \"memory_split\" has a value of %d or greater\n",
        gpu_mem, min_gpu_mem);
  else if (!detected)
    vcos_log_error("Camera is not detected. Please check carefully the camera "
